package com.company.smoketestdashboard.util;

public class Constants {

    public static final String ADD_TEST_SUITE = "add-test-suite";
    public static final String DELETE_TEST_SUITE = "delete-test-suite";
    public static final String MODIFY_TEST_SUITE = "modify-test-suite";
    public static final String RUN_TEST_SUITE = "run-test-suite";
    public static final String CONTEXT_ROOT = "enterprise/test/smtd/v1";
    public static final String HEALTH_CHECK = "healthcheck";
    public static final String TEST_SUITE_ADDED_STRING = "ADDED";
    public static final String FAILURE_STRING = "FAILED";
    public static final String TEST_SUITE_DELETED_STRING = "DELETED";
    public static String DATETIME_FORMAT = "dd-MMM-yyyy hh.mm.ss.SSS a";
    public static String NOT_YET_EXECUTED = "NOT YET EXECUTED";

}
